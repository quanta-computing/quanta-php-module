...
zend_execute_data *execute_data TSRMLS_DC) {
  ...
  block->name = magento_get_block_attr("\0*\0_nameInLayout",
    sizeof("\0*\0_nameInLayout") - 1, this TSRMLS_CC);
  block->class = magento_get_block_class_name(this TSRMLS_CC);
  if (block->class)
    block->class_file = magento_get_block_class_file(this TSRMLS_CC);
  block->template = magento_get_block_attr("\0*\0_template",
    sizeof("\0*\0_template") - 1, this TSRMLS_CC);
  if (!block->name) {
