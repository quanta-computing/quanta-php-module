...
int qm_begin_profiling(const char *curr_func, zend_execute_data *execute_data) {
  ...
  if (!hp_globals.profiled_application) {
    hp_globals.profiled_application = qm_match_first_app_function(curr_func, execute_data);
    if (!hp_globals.profiled_application)
      return -1;
  ...
  function = hp_globals.profiled_application->match_function(curr_func, execute_data);
  ...
  && function->begin_callback(hp_globals.profiled_application, function, execute_data)) {
    return -1;
  }
...
int qm_end_profiling(int function_idx, zend_execute_data *execute_data) {
  ...
  && function->end_callback(hp_globals.profiled_application, function, execute_data)) {
    function_idx = -1;
  }
  return function_idx;
}
